...
write -format verilog -hier -output [format "%s%s%s%s" $top_module .out.v]

# Write Reports
redirect [format "%s%s%s%s" log/ $top_module _area.rep] { report_area }
...
#redirect -append [format "%s%s%s%s" log/ $top_module _area.rep] { report_reference }
